...
<body>
	...
	<div class="wrapper">
		<div class="start">
				...
				<li><a id="active" href="conferences.php">CONFERENCES</a></li>
...
$sql = "select * from event where event_type='conference'";
...
while($row=$result->fetch())
{
	...
	echo '<tr><td><input type="text" size="15" style="border: none; color: grey; font-size: 10px" name="event_name'.$i.'" value="'.$row["event_name"].'" ></td>
		  ...
		  <td><input type="submit" name="add'.$i.'" size="5" name="add" value="add"></td>
...
for($a=1;$a<=$i;$a++)
{
	...
	$j=$_POST['location'.$a];
		...
		$statement->bindvalue(3,"conference");
		...
		{
			echo "addition";
			header("Refresh: 5; url= conferences.php");				
		}
